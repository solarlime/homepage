...
const situations: Situation[] = [
  ...
  {
    maintenance: false,
    path: `/${import.meta.env.VITE_APP_PLEASE}`,
    searchFor: 'Bb',
  },
...
describe.each(situations)(
  'Page resolving',
  ({ maintenance, path, searchFor }: Situation) => {
    beforeEach(() => {
      import.meta.env.VITE_APP_MAINTENANCE_MODE = maintenance;
    });

    test(`Maintenance: ${maintenance} & path: ${path}`, async () => {
      const { getByText } = renderWithProviders(createElement(App, null), {
