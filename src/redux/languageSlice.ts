...
export const languageSlice = createSlice({
  ...
  initialState,
  reducers: {
    toggleLanguage: (state) => setContext(state, languages.ru, languages.en, 'language'),
  },
});

export const { toggleLanguage } = languageSlice.actions;
export const selectLanguage = (state: RootState) => state.language as Language;
export const selectLanguageName = (state: RootState) => (state.language as Language).name;
