 ...
 */
export const shuffleArray = <T extends Array<string> | Array<number>>(
  array: T,
): T => {
...
const TagCloud = memo(
  (props: {
    ...
    const colorsRandom = useMemo(
      () => (Array.isArray(tags) ? randomArray(tags.length) : []),
      [tags],
    );
        ...
        <ul className={styles['tag-cloud']}>
          {shuffledTags.map((item, i) => (
            <li
              ...
              key={id()}
              style={{
                color: `${colorsRandom[i] < 0.5 ? theme.extraColor : theme.accentColor}`,
              }}
            >
              {item}
            </li>
