      ...
      <li className={styles.projects_list__item} data-testid="project">
        ...
        <p
          onClickCapture={
            ...
            (event) => {
              if (
                focused &&
                (event.target as HTMLParagraphElement).querySelector('a') ===
                  focused
              ) {
                window.open(
                  `https://${import.meta.env.VITE_APP_LINK_GITHUB}/${kebabedProjectName}`,
                  '_blank',
                  'noreferrer',
                );
                focused.blur();
        ...
        >
          ...
          <picture>
            ...
            />
            <source
              srcSet={`${source}/${kebabedProjectName}.avif`}
              type="image/avif"
            />
            <img
              ...
              onClick={(event) => {
                ...
                try {
                  ...
                  const link =
                    picture.previousElementSibling as HTMLAnchorElement;
                  link.focus();
                  focused = link;
                } catch (e) {
                  console.error(
                    `An error with '${projectName}' occurred! Fallback mode is on`,
                  );
                  window.open(
                    `https://${import.meta.env.VITE_APP_LINK_GITHUB}/${kebabedProjectName}`,
                    '_blank',
